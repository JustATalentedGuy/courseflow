...
from app.services.transcript import _caption_segments_to_transcript
...
def test_caption_normalization_skips_empty_noise_segments():
    transcript = _caption_segments_to_transcript(
        ...
        [
            ...
            {"start": 1, "duration": 2, "text": "Welcome to the lesson."},
            {"start": 3, "duration": 1, "text": "♪"},
        ],
    )

    assert transcript.full_text == "Welcome to the lesson."
    assert len(transcript.segments) == 1


def test_caption_normalization_rejects_all_noise_segments():
    with pytest.raises(TranscriptValidationError, match="no valid speech segments"):
        _caption_segments_to_transcript(
            "video-edge",
            "youtube_captions",
            "en",
            [
...
@pytest.mark.asyncio
async def test_expired_leased_edge_job_is_claimable_again(client, db_session):
    access_token = await register_and_login(client, "edge-expired@example.com")
    token_response = await client.post(
        "/api/v1/edge/tokens",
        headers={"Authorization": f"Bearer {access_token}"},
        json={"name": "Laptop"},
    ...
    claim = await client.post(
        "/api/v1/edge/jobs/claim",
        headers={"Authorization": f"Bearer {edge_token}"},
        json={"worker_id": "new-worker", "limit": 1},
    )

    assert claim.status_code == 200
    job = claim.json()["jobs"][0]
    assert job["youtube_video_id"] == "video-edge"
    assert job["lease_token"] != "old-lease"
